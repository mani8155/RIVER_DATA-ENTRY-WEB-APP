...
def cadastral_master_kml(request: Request, river_name: str):
    ...
    obj = CadastralMasterKMLTable.objects.get(river_name=river_name_id)
    # print(obj)
        ...
        try:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    error_message = f"No ListOFValues object found with River Name"
                    print(e)
                    response = {"error": error_message}
                    return Response(data=response, status=status.HTTP_400_BAD_REQUEST)
            ...
            for index, row in df.iterrows():
                ...
                for query in query_list:
                    try:
                        ...
                        response = {"error": f"Not found in {query} ListOFValues : {row[query]}"}
                        return Response(data=response, status=status.HTTP_400_BAD_REQUEST)
                    except ListOFValues.MultipleObjectsReturned:
                        response = {"error": f"Multiple ListOFValues objects found with {query}: {row[query]}"}
                        return Response(data=response, status=status.HTTP_400_BAD_REQUEST)
                ...
                for street_obj in street_list:
                    try:
                        ...
                        if not query_obj:
                            response = {"error": f"No Street object found with {street_obj}: {row[street_obj]}"}
                            return Response(data=response, status=status.HTTP_400_BAD_REQUEST)
                ...
                try:
                    ...
                    if not query_obj:
                        response = {"error": f"No Street object found with {survey_no}: {row[survey_no]}"}
                        return Response(data=response, status=status.HTTP_400_BAD_REQUEST)
                except SurveyMaster.DoesNotExist:
                    response = {"error": f"No Street object found with {survey_no}: {row[survey_no]}"}
                    return Response(data=response, status=status.HTTP_400_BAD_REQUEST)
